...
use shared::common::network::packet_type::ServerPacketType::Chunk;
use shared::common::network::server::quit_packet::QuitPacket;
use crate::server::network::socket::ServerConnection;
...
impl ClientConnection {
    pub async fn start(packet : ClientPacket, socket : ServerConnection, server_world_data: Arc<ServerWorldData>) {
        let (psx, prx) = mpsc::channel::<ServerPacket>(100000);
        ...
        socket.send(ServerPacket::Connect(ConnectionPacket::new(pos))).await.unwrap();
    ...
    pub async fn handle_client(&mut self) {
            ...
            tokio::select! {
                ...
                packet = self.socket.recv() => {
                    if let Err(e) = self.receive(&packet) {
                        print_base!("Exiting {} due to error {}", self.socket.get_addr(), e);
                        break;
                    } else {
                        instant = Instant::now();
                    }
                ...
                Some(packet_data) = self.prx.recv() => {
                    ...
                    self.socket.send(packet_data).await.unwrap();
                }
                ...
                _ = heartbeat_interval.tick() => {
                    ...
                    if instant.elapsed().gt(&Duration::from_secs(5)) {
                        print_base!("Exiting {} due to time elapsed", self.socket.get_addr());
                        break;
                    }
        ...
        self.socket.send(ServerPacket::Quit(QuitPacket::new())).await.expect("");
    ...
    fn load_chunks(arc_chunk_map :  Arc<RwLock<ServerChunkMap>>, server_player: Arc<RwLock<ServerPlayer>>, last_pos : ChunkPos, new_pos : ChunkPos, old_view_distance : i32, new_view_distance : i32) {
        ...
        let flattened_pos = new_pos.flattened();
        let (v1, v2 ) = ServerChunkMap::compute_chunk_diff(last_pos, new_pos, old_view_distance, new_view_distance);
        print_base!("Chunks to load {}", v1.len());
        for pos in v1 {
