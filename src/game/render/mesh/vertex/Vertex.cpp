...
uint64_t Vertex::packData(const int id, const glm::ivec3 pos, const glm::vec3 lighting, const int face_id, const unsigned char texCoords) {
    if(id >= pow(2,18))
    ...
    packed |= (uint64_t)((int) (lighting.x * 15.f + 0.5f) & 0xF) << 15; // Ambient lighting (4 bits)
    packed |= (uint64_t)((int) (lighting.y * 15.f + 0.5f) & 0xF) << 11; // Diffuse lighting (4 bits)
    packed |= (uint64_t)((int) (lighting.z * 15.f + 0.5f) & 0xF) << 7; // Specular lighting (4 bits)
